...
function chooseHole(holes) {
  const holeIndex = randomInteger(0, 8);
...
/**
 *
 * Adds or removes the 'end' class that is defined in styles.css to
 * change the timer color when the game is close to the end.
 *
 */
function toggleTimerColor() {
  if (time > 0 && time <= 3) timerDisplay.classList.add("end");
  else timerDisplay.classList.remove("end");
}
...
/**
 *
 * This function displays "game over" message when time runs out.
 *
 */
